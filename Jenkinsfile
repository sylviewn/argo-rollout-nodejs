pipeline {
    ...
    environment {
    DOCKERHUB_CREDENTIALS = credentials('sylviewette')
    APP_NAME = "sylviewette/bluegreen-rolloout-oct"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/sylviewn/Argo-rollout-nodejs.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
